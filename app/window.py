...
class GLFWWindow:
    def __init__(
        ...
        # compensamos spacing inicial con gap y grosor extra
        ...
        if self.live_mode:
            ...
            try:
                self.ohlc: list[OHLC] = rest.fetch_klines(
                    symbol=self.live_symbol,
                    interval=self.live_interval,
                    limit=self.history_limit,
                )
                print(f"[REST] {len(self.ohlc)} velas cargadas")
            except Exception as e:
                print(f"[REST][ERROR] {e}")
                self.ohlc = []
        else:
            self.ohlc = make_fake_ohlc(
                400,
                start_price=100.0,
                volatility=1.2,
                seed=7,
        ...
        self.series = CandleSeries(self.ohlc, style=style)
        ...
        self.chart.add_overlay(self.chart_overlay, layer="base")
        self.chart.add_overlay(self.price_axis_overlay, layer="base")
        self.chart.add_overlay(self.time_axis_overlay, layer="base")
        self.chart.add_series(self.series)
        self.chart.add_overlay(self.crosshair_overlay, layer="front")
    ...
    def init(self) -> None:
        ...
        self.text_renderer = TextRenderer(
            font_path=r"C:\Users\ozzyj\OneDrive\Escritorio\Programacion\libreria_grafica_openGL\font\fonts_fft\Montserrat SemiBold 600.ttf",
            font_size=11,
            width=fb_w,
            height=fb_h,
        )
        ...
        self.chart.add_overlay(self.tooltip_overlay, layer="front")
    ...
    def update(self) -> None:
        ...
        if (
            not self._price_manual_mode
            and vr.end_idx >= vr.start_idx
            and len(self.series.data) > 0
        ):
            self.price_scale.autoscale_from_provider(
                vr.start_idx,
                vr.end_idx,
    ...
    def _update_live_feed(self) -> None:
        ...
        events = self.live_feed.poll_events(limit=200)
        if not events:
            return
        ...
        for ev in events:
            if ev.type == "bar":
                ...
                status = ev.payload
                print(f"[Feed][{status.state}] {status.message}")

            elif ev.type == "error":
                err = ev.payload
                print(f"[Feed][ERROR] {err.message}")

        if changed:
            self.time_scale.set_timestamps([c.ts for c in self.series.data])
