...
class ChartOverlay:
    """
    Solo layout + separadores.
    NO dibuja bandas de fondo (rectángulos negros) para los ejes.
    Los ticks, labels y gridline_in_plot los dibuja después el axis overlay.
    """
    ...
    DEFAULT_CONFIG = {
        ...
        "colors": {
            "axis_separator": (0.4, 0.4, 0.4, 0.8),     # gris tenue
        },
        "coords": {"y_down": True},  # origen arriba (UI común) — cambiar a False si y crece hacia arriba
    }

    def __init__(self, time_scale, price_scale, config: Optional[Dict[str, Any]] = None):
    ...
    def set_view(self, x: float, y: float, w: float, h: float) -> None:
        """Define el rectángulo total asignado al chart."""
        self._x, self._y, self._w, self._h = float(x), float(y), float(w), float(h)
        self._layout = None  # invalidate cache

    def get_layout(self) -> OverlayLayout:
        """Calcula y cachea el layout."""
        if self._layout is None:
            self._layout = self._compute_layout()

            # Conectar escalas al área de plot (no al chart entero)
            px, py, pw, ph = self._layout.plot_rect
            self.time_scale.set_view(px, pw)  # asume que TimeScale tiene este método
    ...
    def get_plot_rect(self) -> Rect:
        """Rectángulo útil para dibujar series, crosshair, etc."""
        return self.get_layout().plot_rect

    def draw(self, renderer) -> None:
        """Dibuja solo los separadores (líneas grises)."""
    ...
    def _compute_layout(self) -> OverlayLayout:
        ...
        time_rect: Rect = (
            (inner_x, time_y, inner_w, time_h) if ta["show"] else (0.0, 0.0, 0.0, 0.0)
        )

        # Price axis (derecha o izquierda)
        ...
        if side == "left":
            ...
            plot_rect: Rect = (inner_x, plot_y, max(0.0, inner_w - price_w), plot_h)
            price_rect: Rect = (inner_x + plot_rect[2], plot_y, price_w, plot_h) if pa["show"] else (0.0, 0.0, 0.0, 0.0)
    ...
    def _draw_price_axis_separator(self, renderer, axis_rect: Rect, plot_rect: Rect) -> None:
        ...
        side = self.config["price_axis"]["side"].lower()
        sep_w = self.config["price_axis"]["separator_width"]
    ...
    def _draw_time_axis_separator(self, renderer, axis_rect: Rect, plot_rect: Rect) -> None:
        ...
        px, py, pw, ph = plot_rect
        sep_w = self.config["time_axis"]["separator_width"]
        sep_col = self.config["colors"]["axis_separator"]
