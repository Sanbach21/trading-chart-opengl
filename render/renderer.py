...
from OpenGL.GL import (
    GL_ARRAY_BUFFER, GL_BLEND, GL_DYNAMIC_DRAW, GL_FLOAT,
    GL_FRAGMENT_SHADER, GL_LINES, GL_ONE_MINUS_SRC_ALPHA, GL_SRC_ALPHA,
    GL_TRIANGLES, GL_VERTEX_SHADER, glBindBuffer, glBindVertexArray,
    glBlendFunc, glBufferData, glDeleteBuffers, glDeleteProgram,
    glDeleteVertexArrays, glDrawArrays, glEnable, glEnableVertexAttribArray,
    glGenBuffers, glGenVertexArrays, glGetUniformLocation, glUseProgram,
    glUniform2f, glVertexAttribPointer,
)
...
class Renderer2D:
    ...
    def __init__(self) -> None:
        ...
        self._line_verts: List[float] = []  # legacy (GL_LINES)
        self._tri_verts: List[float] = []   # rects + líneas gruesas como quads
    ...
    def init(self) -> None:
        ...
        glVertexAttribPointer(1, 4, GL_FLOAT, False, stride, ctypes.c_void_p(8))
    ...
    def shutdown(self) -> None:
        if self._program is not None:
            glDeleteProgram(self._program); self._program = None
        if self._vbo is not None:
            glDeleteBuffers(1, [self._vbo]); self._vbo = None
        if self._vao is not None:
            glDeleteVertexArrays(1, [self._vao]); self._vao = None
    ...
    def end_frame(self) -> None:
        ...
        # Triángulos (rects + líneas gruesas)
        if self._tri_verts:
        ...
        # Líneas finas legacy (si alguien aún las usa)
        if self._line_verts:
    ...
    def draw_line_px(self, x1: float, y1: float, x2: float, y2: float,
        ...
        """
        Línea en píxeles con GROSOR. No depende de glLineWidth.
        - H/V: usa rect (nítido).
        - Diagonal: genera un quad (dos triángulos) alrededor del segmento.
        """
    ...
    def draw_rect_px(self, x: float, y: float, w: float, h: float, color: Any, **_) -> None:
        """
        Rect relleno (dos triángulos). (x,y) esquina sup-izquierda (coordenadas de app).
        """
        col = self._to_color(color)
        x2 = x + w
        y2 = y + h
    ...
    # -------------------------
    # Internal: quad para línea diagonal
    # -------------------------
